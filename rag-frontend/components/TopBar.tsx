...
export default function TopBar({
  ...
  /*
   * Load the same authenticated user used by Sidebar.
   *
   * This removes the old hardcoded "LS".
   */
  useEffect(() => {
    let cancelled = false;
  ...
  return (
    <header
      ...
      style={{
        background:
          "linear-gradient(90deg, #6D46EA 0%, #7C5CFC 100%)",
      }}
    >
      {/* =========================================================
          LEFT SIDE
      ...
      <div className="flex items-center gap-5 min-w-0">
        ...
        {mode === "global" ? (
          <span className="text-[16px] font-bold text-white tracking-tight shrink-0">
            ScholarAI
          </span>
        ) : (
          <div className="flex items-center gap-3 min-w-0">
            <span className="text-[16px] font-bold text-white tracking-tight truncate">
              {sessionTitle}
            </span>

            {typeof sourcesActiveCount ===
              "number" && (
