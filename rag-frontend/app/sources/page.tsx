      ...
      <main className="flex-1 flex flex-col min-w-0">
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar px-5 sm:px-10 py-8">
          <div className="max-w-5xl mx-auto">
            <h1 className="text-[26px] sm:text-[30px] font-bold text-lime-300 tracking-tight mb-2">Knowledge Sources</h1>
            <p className="text-[14.5px] text-gray-400 mb-8">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-[1.1fr_1fr] gap-5 mb-10">
              ...
              <button
                ...
                className={`${CARD} flex flex-col items-center justify-center text-center px-8 py-10 hover:border-zinc-700 hover:bg-zinc-900 transition-colors`}
              >
                ...
                <p className="text-[16px] font-bold text-white mb-1.5">Upload Documents</p>
              ...
              <div className="flex flex-col gap-5">
                ...
                <div className={`${CARD} p-5 opacity-50`}>
                  ...
                  <div className="flex items-center gap-2">
                    <input
                      ...
                      className="flex-1 min-w-0 bg-black/40 border border-zinc-800 rounded-xl px-3.5 py-2.5 text-[13px] text-gray-500 placeholder:text-gray-600 outline-none cursor-not-allowed"
                    />
                    <button disabled className="shrink-0 px-4 py-2.5 rounded-xl bg-zinc-800 text-gray-500 text-[13px] font-bold cursor-not-allowed">
                      Add
                    </button>
                  </div>
                </div>
                ...
                <div className={`${CARD} p-5 flex-1 flex flex-col opacity-50`}>
                  ...
                  <textarea
                    ...
                    rows={3}
                    className="w-full flex-1 resize-none bg-black/40 border border-zinc-800 rounded-xl px-3.5 py-3 text-[13px] text-gray-500 placeholder:text-gray-600 outline-none cursor-not-allowed mb-3"
                  />
                  <button disabled className="self-end px-4 py-2 rounded-xl border border-zinc-800 text-gray-500 text-[12.5px] font-semibold cursor-not-allowed">
                    Import
                  </button>
                </div>
              </div>
            ...
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-3">
                <h2 className="text-[16px] font-bold text-white">Active Repository</h2>
            ...
            {uploadingJobs.length > 0 && (
              <div className="space-y-2 mb-3">
                {uploadingJobs.map((job) => (
                  <div key={job.id} className="flex items-center gap-3.5 px-4 py-3.5 rounded-2xl bg-zinc-900/50 border border-zinc-800/80">
                    <div className="w-9 h-9 rounded-lg bg-zinc-800 flex items-center justify-center shrink-0 text-gray-300">
                      <IconFile width={16} height={16} />
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="text-[13.5px] font-medium text-gray-100 truncate m-0">{job.name}</p>
            ...
            <div className="space-y-2">
              ...
              {!loading && !loadError && sources.length === 0 && uploadingJobs.length === 0 && (
                <p className="text-[13.5px] text-gray-500 italic py-6 text-center border border-dashed border-zinc-800 rounded-2xl">
                  No sources yet — upload a PDF above to get started.
                </p>
              )}
              {sources.map((item) => (
                <div key={item.id} className="flex items-center gap-3.5 px-4 py-3.5 rounded-2xl bg-zinc-900/50 border border-zinc-800/80">
                  <div className="w-9 h-9 rounded-lg bg-zinc-800 flex items-center justify-center shrink-0 text-gray-300">
                    <IconFile width={16} height={16} />
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-[13.5px] font-medium text-gray-100 truncate m-0">{item.filename}</p>
                    <p className="text-[11.5px] text-gray-500 truncate m-0 mt-0.5">
